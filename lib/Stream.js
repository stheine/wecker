...
class Stream {
  constructor({rpio}) {
    ...
    // TODO ich koennte alle play() aufrufe auf die playQueue umstellen
  ...
  async getPodcastList(podcast) {
    const podcastList = await new Promise(resolve => {
      const subProcess = childProcess.fork('lib/getPodcastList.js', [podcast.url]);
  ...
  async playPodcastList(podcastList) {
    this.playQueue = _.clone(podcastList);
