        ...
        switch(action) {
          case 'list': {
            ...
            if(_.get(this.menus, [podcastListMenuLabel, 'refreshed'], moment().subtract(1, 'hour'))
              ...
              let podcastList = await this.stream.getPodcastList(option);

              podcastList = _.filter(podcastList, podcast =>
                moment(podcast.pubDate._text).isAfter(moment().subtract(option.maxAge)));
