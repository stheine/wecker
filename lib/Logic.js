...
class Logic {
  ...
  findNextAlarm(alarms) {
    ...
    do {
      const dayName      = dayjs.weekdays()[checkDayNum];
      const dayAlarm     = _.get(alarms, [dayName]);

      if(dayAlarm &&
        dayjs(`${dayName} ${dayAlarm}`, 'dddd h:mm').isAfter(currentTime) &&
        dayjs(`${dayName} ${dayAlarm}`, 'dddd h:mm').isBefore(displayEndTime)
      ) {
        return `${dayName.slice(0, 3)} ${dayAlarm}`;
      }
