...
class Logic {
    ...
    switch(target) {
      case 'menu':
        switch(action) {
          ...
          case 'open': this.menuStack.push(this.activeMenu); this.activeMenu = this.menus[option]; break;
      ...
      case 'stream':
        switch(action) {
          case 'toggle': this.stream.toggle(); break;
  ...
  press1() {
    ...
    switch(this.display) {
      case 'menu':   this.activeMenu.press(); break;
      case 'time':   this.activeMenu = this.menus.top; this.display = 'menu'; break;
