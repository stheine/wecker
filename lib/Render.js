...
const font12 = require('../fonts/oled-js-font-Consolas-12-190');
...
const lineHeight = font12.height;
const padding = 4;
const numShow = 4;
...
class Render {
  ...
  async renderMenu(params) {
    ...
    for(let key = 0; key < entries.length; key++) {
      ...
      if(key === activeEntry) {
        this.oled.writeString(padding, y + 0, font12, entry.label, 'WHITE', false);
  ...
  renderScrollText(text) {
    ...
    this.oled.drawLine(x + text.length * font16.width, 32, x + text.length * font16.width, 32 + font16.height, 'BLACK');
  }
  ...
  async refresh(params) {
    ...
    if(this.infoLargeScrollInterval &&
      ...
      await this.oled.fillRect(0, 32, 127, 48, 'BLACK', true);
    }
